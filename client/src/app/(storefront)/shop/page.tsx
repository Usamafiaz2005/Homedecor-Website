...
// Premium Curated Metadata Grid Fallbacks for Architectural Consistency
...
export default function ShopPage() {
  ...
  return (
    <main className="bg-luxury-white min-h-screen pt-20">
      {/* Immersive Editorial Header Banner */}
      <div className="relative h-[40vh] w-full overflow-hidden bg-charcoal">
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-40 mix-blend-luminosity scale-105 transition-transform duration-[4000ms]"
        ...
        <div className="relative z-10 h-full max-w-7xl mx-auto px-6 flex flex-col justify-center items-center text-center">
          ...
          <motion.h1 
            ...
            className="text-3xl md:text-5xl font-serif text-charcoal tracking-wide capitalize"
          >
            {category ? `${category} Collection` : currentBanner.title}
          </motion.h1>
          <p className="text-charcoal/60 font-sans text-xs max-w-md mt-4 leading-relaxed font-light">
            {currentBanner.subtitle}
          </p>
        </div>
      </div>

      {/* Main Interactive Retail Architecture Section */}
      <section className="max-w-7xl mx-auto px-6 py-12">
        {/* Breadcrumb Navigation trail */}
        <div className="text-[10px] uppercase tracking-widest text-charcoal/40 mb-8 flex items-center gap-2 font-sans">
          <a href="/" className="hover:text-walnut-brown transition-colors">Home</a>
          <span>/</span>
          <span className="text-charcoal/80 font-medium">Shop</span>
        </div>

        {/* Dynamic Client Grid Layer mapping internal backend state */}
